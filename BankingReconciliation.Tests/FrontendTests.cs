...
public class FrontendTests : IClassFixture<BankingReconciliationWebApplicationFactory>
{
    ...
    public async Task Root_ReturnsFrontendHtml()
    {
        ...
        Assert.Contains("demo-session-button", html);
        Assert.Contains("Sunumu tek tıkla hazırla", html);
        Assert.Contains("advanced-settings-toggle", html);
        Assert.Contains("workflow-guide", html);
        Assert.Contains("/api/demo-files/comparison-file-1", html);
        Assert.Contains("Hatalı dosya", html);
        Assert.Contains("load-valid-demo-files", html);
        Assert.Contains("load-invalid-demo-files", html);
    ...
    public async Task AppJs_IncludesTurkishStatusDescriptions()
    {
        ...
        Assert.Contains("prepareDemoSession", script);
        Assert.Contains("preparePresentation", script);
        ...
        Assert.Contains("loadDemoFiles", script);
        Assert.Contains("assignFileToInput", script);
        ...
        Assert.Contains("/api/demo-auth/session", script);
